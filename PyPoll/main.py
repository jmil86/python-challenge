# -*- coding: UTF-8 -*-
"""PyPoll Homework Starter File."""

# Import necessary modules
import csv
import os

# Files to load and output (update with correct file paths)
file_to_load = os.path.join("Resources", "election_data.csv")  # Input file path
file_to_output = os.path.join("analysis", "election_analysis.txt")  # Output file path

# Initialize variables to track the election data
total_votes = 0  # Track the total number of votes cast

# Define lists and dictionaries to track candidate names and vote counts
list_candidate = []
list_candidate_vote_count = []

# Winning Candidate and Winning Count Tracker
cand_most_votes = 0
cand_most_name = ''

# Open the CSV file and process it
with open(file_to_load) as election_data:
    reader = csv.reader(election_data)

    # Skip the header row
    header = next(reader)

    # Loop through each row of the dataset and process it
    for row in reader:
        # Increment the total vote count for each row
        total_votes += 1

        # Get the candidate's name from the row
        candidate_name = row[2]

        # Check if the candidate is already in the candidate list
        if candidate_name not in list_candidate:
            list_candidate.append(candidate_name)
            list_candidate_vote_count.append({'Name': candidate_name, 'Vote Count': 0})

        # Increment the vote count for the candidate
        for candidate in list_candidate_vote_count:
            if candidate['Name'] == candidate_name:
                candidate['Vote Count'] += 1
                break

# Open a text file to save the output
with open(file_to_output, "w") as txt_file:

    # Print the total vote count (to terminal)
    print("Election Results")
    print(f"|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|")
    print(f"Total Votes: {total_votes:,}")
    print(f"|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|")

    # Write the total vote count to the text file
    txt_file.write("Election Results\n")
    txt_file.write(f"|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|\n")
    txt_file.write(f"Total Votes: {total_votes:,}\n")
    txt_file.write(f"|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|\n")

    # Loop through the candidates to determine vote percentages and identify the winner
    for candidate in list_candidate_vote_count:
        # Get the vote count and calculate the percentage
        cand_vote_percent = candidate['Vote Count'] / total_votes
        cand_vote_count = candidate['Vote Count']

        # Update the winning candidate if this one has more votes
        if cand_vote_count > cand_most_votes:
            cand_most_votes = cand_vote_count
            cand_most_name = candidate['Name']

        # Print and save each candidate's vote count and percentage
        print(f"{candidate['Name']}: {cand_vote_percent:.2%} ({cand_vote_count:,})")
        txt_file.write(f"{candidate['Name']}: {cand_vote_percent:.2%} ({cand_vote_count:,})\n")

    # Generate and print the winning candidate summary
    print(f"Winner: {cand_most_name}")

    # Save the winning candidate summary to the text file
    txt_file.write(f"Winner: {cand_most_name}\n")